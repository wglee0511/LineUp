...
class UserFragment : Fragment() {
        ...
        firestore?.collection("users")?.document(uid!!)
            ?.addSnapshotListener { documentSnapshot, firebaseFirestoreException ->
                ...
                if (followDTO?.followingCount != null) {
                    fragmentView?.accont_tv_following_count?.text =
                        followDTO?.followingCount?.toString()
                }
                if (followDTO?.followCount != null) {
                    fragmentView?.accont_tv_follower_count?.text =
                        followDTO?.followCount?.toString()
    ...
    fun requestFollow() {
        ...
        firestore?.runTransaction { transaction ->
            ...
            if (followDTO!!.followers.containsKey(currentUserUid)) {
                //It cancel my follwer when I follow third person
        ...
        init {
            firestore?.collection("images")?.whereEqualTo("uid", uid)
                ?.addSnapshotListener { querySnapshot, firebaseFirestoreException ->
                    ...
                    fragmentView?.accont_tv_post_count?.text = contentDTOs.size.toString()
